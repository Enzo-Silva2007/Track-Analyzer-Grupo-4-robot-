function extraerSSL(respuestaRed) {
  ...
  return {
    valido:        true,
    ca:            cert.issuer(),
    protocolo:     cert.protocol(),    // TLS 1.2, TLS 1.3
    dominio:       cert.subjectName(), // para qué dominio fue emitido
    emision,                           // desde cuándo es válido
    vencimiento,                       // hasta cuándo es válido
    duracionDias,                      // duración total del certificado
    diasRestantes,
    alerta:        diasRestantes <= 30 ? `Vence en ${diasRestantes} días` : null,
  };
}

module.exports = extraerSSL;